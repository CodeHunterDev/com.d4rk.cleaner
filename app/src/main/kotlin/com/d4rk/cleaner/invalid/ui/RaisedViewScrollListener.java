...
public class RaisedViewScrollListener extends RecyclerView.OnScrollListener {
    ...
    public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
        ...
        if (recyclerView.getLayoutManager() instanceof LinearLayoutManager) {
            ...
            throw new IllegalArgumentException(RaisedViewScrollListener.class.getSimpleName() + " currently supports LinearLayoutManager only.");
        }
        ...
        if (mElevationAnimator == null || !mElevationAnimator.isRunning() || mTargetView.getElevation() != getElevationInState(shouldRaise)) {
            mElevationAnimator = ObjectAnimator.ofFloat(mTargetView, "elevation", getElevationInState(!shouldRaise), getElevationInState(shouldRaise)).setDuration(mDuration);
