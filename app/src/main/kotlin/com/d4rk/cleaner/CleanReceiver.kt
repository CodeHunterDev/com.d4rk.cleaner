...
import com.d4rk.cleaner.ScheduledService.Companion.enqueueWork
class CleanReceiver: BroadcastReceiver() {
        override fun onReceive(ctxt: Context, i: Intent) {if (i.action == null) { enqueueWork(ctxt) } else { scheduleAlarm(ctxt) } }
        companion object {
                private
                ...
                fun scheduleAlarm(context: Context) {
                        ...
                        mgr.setRepeating(
                                AlarmManager.ELAPSED_REALTIME, SystemClock.elapsedRealtime() + INITIAL_DELAY, PERIOD.toLong(), pi)
                }
                @JvmStatic
